...
import { authenticationCheckServer } from '@/cookie-server'
...
export async function POST(req: NextRequest) {
  const auth = await authenticationCheckServer()
  if (auth?.authenticated === false) return NextResponse.json({ error: 'Authentication required' }, { status: 401 })
  const accessToken = auth?.ejentoAccessToken
  let headers = {}
  ...
  if (!id) {
    return NextResponse.json({ error: 'Missing corpus ID' }, { status: 400 })
  }
  try {
    ...
    const response = await axios.post(url, axiosFormData, { headers })
